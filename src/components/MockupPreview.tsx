...
import { Smartphone, MapPin, QrCode, MessageCircle, Star, ShoppingBag } from 'lucide-react';
...
export default function MockupPreview() {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-2">
      <div className="transition-all duration-500 ease-in-out border-[8px] border-slate-700 rounded-[2rem] shadow-2xl relative bg-black overflow-hidden w-[260px] h-[450px]">
          ...
          {modules.length === 0 ? (
            ...
            <div className="space-y-0">
              {modules.map((module: any) => (
                <div key={module.id} className="w-full animate-in fade-in slide-in-from-bottom-2 duration-300">
    ...
    case 'Hero':
      return (
        <section className="bg-gradient-to-br from-owned to-indigo-600 p-6 pt-10 text-center text-white relative overflow-hidden">
          ...
          <div className="bg-white/20 backdrop-blur-md px-4 py-2 rounded-full inline-block text-[10px] font-black uppercase">Claim Offer</div>
        </section>
      );
    case 'Map':
      return (
        <section className="p-4 bg-sidebar">
          ...
          <p className="text-[9px] text-text-dim bg-black/40 p-2 rounded border border-border font-medium">📍 Central Exit B (2 mins walk)</p>
        </section>
      );
    case 'Chatbot':
      return (
    ...
    case 'CTA':
      return (
        <section className="p-4 flex justify-center">
          <button className="w-full bg-accent text-bg font-black py-3 rounded-xl uppercase tracking-widest text-[11px]">Get Started Now</button>
        </section>
      );
    case 'Form':
      return (
        <section className="p-4 space-y-3">
          ...
          <button className="w-full bg-slate-800 text-white font-black py-2 rounded uppercase text-[10px]">Submit</button>
        </section>
      );
    case 'Subscription':
      return (
        <section className="p-6 bg-owned/10 border-y border-owned/20 text-center">
          ...
          <div className="flex gap-1">
            ...
            <div className="bg-owned px-3 py-1 rounded text-[9px] font-black flex items-center">OK</div>
          </div>
        </section>
      );
    case 'Redemption':
      return (
        <section className="p-6 text-center">
          <div className="bg-gradient-to-br from-physical to-red-600 p-4 rounded-3xl text-white shadow-xl">
            ...
            <h4 className="text-sm font-black mb-4 uppercase tracking-tighter italic">Exclusive Reward</h4>
            <div className="bg-black/20 py-3 rounded-lg border border-white/10 mb-2">
              <p className="text-lg font-mono font-black tracking-widest leading-none">O2O-PASS</p>
            </div>
          </div>
        </section>
      );
